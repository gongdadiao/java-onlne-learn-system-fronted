<template>
  <a-layout>
    <a-layout-header class="header" style="color: black">
      ...
      <span style="float: right;" v-if="status!='done'">
      ...
      <span v-else>
        您的分数:{{examDetail.exam.examScore}}
      </span>
<!--      , position: 'fixed'-->
    </a-layout-header>
    <a-layout>
      <a-layout-sider width="190" :style="{background: '#444',overflow: 'auto', height: '100vh', left: 0 }">
        <a-menu
          mode="inline"
            ...
            <div v-else>
              <div v-if="status=='done'">
                ...
                <br/>
                  <div>
                    成绩分布
                    <div style="width: 80%;height: 350px;border:1px solid rgb(180,180,180)" id="echartss"></div>
                  </div>
              </div>
            </div>
          </div>
...
import {getGradeDistribute} from '../../api/resource'
...
export default {
  ...
  methods: {
    setECharts(){
      var that=this
      getGradeDistribute(this.$route.params.id)
        .then(resp=>{
          if(resp.code==0){
            var dataList=resp.data.items
            for (const item of dataList) {
              if(item.value==0)
                continue
              this.echarts_datas.series[0].data.push({value:item.value,name:item.name+' '+item.value+'人'})
              //this.echarts_datas.legend.data.push(item.name)
              // 基于准备好的dom，初始化echarts实例
              let myChart = echarts.init(document.getElementById('echartss'))
              // 绘制图表，this.echarts1_option是数据
              myChart.setOption(this.echarts_datas)
            }

          }else{
            this.$message.info('获取成绩分布失败')
          }
        })
      ...
      finishExam(this.$route.params.id, JSON.stringify(this.answerPage))
        .then(res => {
          if (res.code === 0) {
            ...
            this.$router.back()//.push('/list/exam-record-list')
            return res.data
          } else {
