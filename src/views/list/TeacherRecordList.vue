...
<script>
  ...
  import { getAdminRecordList } from '../../api/exam'
  ...
  export default {
    ...
    data () {
      return {
        ...
        loadData: parameter => {
          ...
          return getAdminRecordList(this.$props.classId,Object.assign(parameter, this.queryParam))
            .then(res => {
              if (res.code === 0) {
                return res.data
              } else {
    ...
    methods: {
      ...
      handleEdit (record) {
        ...
        console.log(record)
        this.$router.push({path:'/submit/show-test/'+record.examRecordId})
      },
      handleSub (record) {
        // 查看题目，不在模态框里查啦，太麻烦
        // console.log(record)
